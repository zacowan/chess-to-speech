...
HOW_ROOKS_MOVE = ["There is a Rook on {piece_location}. Rooks move horizontally or vertically any number of squares. They are unable to jump over pieces. Rooks move when the king castles."]
...
def handle(session_id, intent_data, board_str):
    ...
    if intent_data.all_required_params_present is True:
        ...
        if(piece == None):
            static_choice = get_random_choice(EMPTY_SPACE_RESPONSE).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "P"):
            static_choice = get_random_choice(HOW_PAWNS_MOVE).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "N"):
            static_choice = get_random_choice(HOW_KNIGHTS_MOVE).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "B"):
            static_choice = get_random_choice(HOW_BISHOPS_MOVE).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "R"):
            static_choice = get_random_choice(HOW_ROOKS_MOVE).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "Q"):
            static_choice = get_random_choice(HOW_QUEEN_MOVES).format(piece_location=piece_location)
            return static_choice, True
        elif(piece.symbol() == "K"):
            static_choice = get_random_choice(HOW_KING_MOVES).format(piece_location=piece_location)
            return static_choice, True
